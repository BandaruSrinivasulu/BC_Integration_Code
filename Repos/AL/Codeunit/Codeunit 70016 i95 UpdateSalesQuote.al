codeunit 70016 "i95 Update SalesQuote"
{
    trigger OnRun()
    begin
        CreateSalesOrder();
    ...
    var
        ...
        targetid: Code[20];
        ContactBusinessRelationL: Record "Contact Business Relation";
    ...
    begin
        ...
        If SalesInputDataJsonObject.Contains('inputData') then begin
            ...
            If SalesInputDataJsonObject.Contains('customer') then begin
                ...
                targetid := i95WebServiceExecuteCU.ProcessJsonTokenasCode('targetId', SOAddressJsonObject);
                firstName := i95WebServiceExecuteCU.ProcessJsonTokenasText('firstName', SOAddressJsonObject);
                lastName := i95WebServiceExecuteCU.ProcessJsonTokenasText('lastName', SOAddressJsonObject);
        ...
        /*  IF ShippingAgentCode <> '' then begin //New Changes for Shipping mapping 
              ...
              Error(StrSubstNo(ShippingAgentMappingErrorTxt, ShippingAgentCode, SalesOrderNo));*///Sales Quote Changes
        //Sales Quote Changes stop
        ...
        If SalesOrderNo <> '' then begin
            ...
            If IsGuest and (i95Setup."Default Guest Customer No." <> '') then
                SalesHeader.validate("Sell-to Customer No.", i95Setup."Default Guest Customer No.")
                     ...
                     begin
                ContactL.Reset();
                ContactL.SetRange("No.", targetid);
                IF ContactL.FindFirst() then begin
                    ContactBusinessRelationL.Reset();
                    ContactBusinessRelationL.SetRange("Link to Table", ContactBusinessRelationL."Link to Table"::Customer);
                    ContactBusinessRelationL.SetRange("Contact No.", ContactL."Company No.");
                    IF ContactBusinessRelationL.FindFirst() then
                      ...
                      IF i95Setup."i95 Enable Company" = true then begin
                          IF customerType = 'Company' then
                              ...
                              if (customerType = 'Customer') or (customerType = '') then
                          ...
                          Customer.SetRange("i95 Reference ID", customerSourceId);*/
                Customer.Reset();
                    Customer.SetRange("No.", ContactBusinessRelationL."No.");
                end else begin
                    Customer.Reset();
                    Customer.SetRange("i95 Reference ID", customerSourceId);
                end;
            ...
            SalesHeader."Sell-to Customer Name" := Copystr(firstName + ' ' + lastName, 1, 99);
            ...
            if SalesInputDataJsonObject.Contains('quoteItems') then begin
                SalesInputDataJsonObject.get('quoteItems', SalesLineInputDataJsonToken);
